...
def counterfactual():
    ...
    # store for reuse (IMPORTANT)
    st.session_state.z_structure = z_structure
    st.session_state.z_pathology = z_pathology
...
with tabs[0]:
    ...
    with col1:
        ...
        alpha = st.slider("Grad-CAM Opacity", 0.0, 1.0, 0.4)
...
with tabs[1]:
    ...
    if "no" in st.session_state:
        ...
        with c1:
            st.image(normalize(img_no), use_container_width=True)
            st.write(f"Classifier No Tumor: {predict(img_no):.8f}")
        
        with c2:
            st.image(normalize(img_yes),use_container_width=True)
    ...
    if animate_cf and "z_structure" in st.session_state:

        st.markdown("### 🎥 Counterfactual Transition")

        z_structure = st.session_state.z_structure
        z_pathology = st.session_state.z_pathology
        ...
        for img in imgs:
            ...
            conf = predict(img)
            text.markdown(f"**Tumor Confidence:** {conf:.4f}")
    ...
    if "sequence" in st.session_state:
        ...
        if animate:
            ...
            for i, img in enumerate(imgs):
                cols[i % 5].image(normalize(img), use_container_width=True)
                cols[i % 5].caption(f"{i/(len(imgs)-1):.2f}")
...
with tabs[3]:
    ...
    if "exp" in st.session_state:
        ...
        with c2:
            st.image(diff[0].numpy().squeeze(), clamp=True,use_container_width=True)
            st.caption("Important Regions")

        with c3:
            st.image(normalize(masked), use_container_width=True)
            st.caption("Masked")
        ...
        st.image(focused, caption="Focused Regions Only", use_container_width=True)